...
def run_auth_test(headed=False):
    print(f"\n[Playwright] Launching browser test for User Authentication (Headed={headed})...")
    status = "FAILED"
    details = ""
    screenshot_b64 = None
    ...
    try:
        with sync_playwright() as p:
            ...
            if "Conduit" in title:
                status = "PASSED"
                details = "Title matched expected value."
            else:
                details = f"Unexpected page title: {title}"
